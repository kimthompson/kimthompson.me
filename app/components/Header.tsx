...
export default function Header() {
  ...
  const pickRandomAddon = () => {
    ...
    let soRandom = lol[Math.floor(Math.random() * lol.length)];
  ...
  return (
    ...
    <div className="flex-auto max-w-sm md:max-w-md lg:max-w-2xl">
      <h1 className="font-heading font-extrabold tracking-tight text-6xl md:text-7xl lg:text-8xl italic text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-teal-500 mb-3">Kim Thompson</h1>
      <p className="text-gray-900 dark:text-slate-100">Web (<a className="text-indigo-500 hover:text-gray-900 dark:hover:text-slate-100" href="/resume">+ more</a>) developer. {pickRandomAddon()}</p>
    </div>
